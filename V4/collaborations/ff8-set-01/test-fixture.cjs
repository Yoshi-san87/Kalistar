...
// Publication tests always start before FF8, even after this set is live.
function priorPublications(cards = existing) {
  return structuredClone(cards.filter(c => c.kind === 'created' && c.profile?.collaboration !== 'FF8'));
}
...
function fixture(published = existing) {
  const ROOT = path.resolve(path.parse(__dirname).root, '__ff8_publication_memory_only__');
  const home = path.join(ROOT, 'V4/collaborations/ff8-set-01'), DATA = path.join(ROOT, 'V4/atelier/data');
  ...
  for (const id of ['ff8-balamb-garden', 'ff8-deling-parade']) put(arena(id), { format: 'png', width: 1536, height: 864 });
...
function builderFixture(published) {
  const f = fixture(published), { ROOT, DATA } = f.L;
  const { createBuilder } = require('./build.cjs');
  f.put(path.join(ROOT, 'V3/donnees/armes.json'), require('../../../V3/donnees/armes.json'));
  f.put(path.join(DATA, 'references.json'), references); f.put(path.join(DATA, 'regression.json'), { passed: true });
  for (const name of ['build.cjs', 'model.cjs', 'compose.jsx', 'compose-one.jsx', 'weapon-fleau.svg', 'weapon-fleau.cjs']) f.seed(path.join(f.home, name), 'IN-MEMORY SOURCE ' + name);
  f.put(path.join(f.home, 'faction.json'), { packedGeometry: { left: 672, top: 829, width: 98, height: 223 } });
  f.put(path.join(f.home, 'flag-FF8-packed.png'), { format: 'png', width: 98, height: 223 });
  f.put(path.join(ROOT, 'V4/collaborations/ff7-set-01/weapon-projectile.png'), { format: 'png', width: 96, height: 95 });
  f.put(path.join(f.home, 'weapon-fleau.png'), { format: 'png', width: 96, height: 95 });
  f.put(path.join(f.bank, 'frame.png'), { format: 'png', width: 897, height: 1497 });
  for (const spec of set.cards) f.put(path.join(f.home, 'illustrations', spec.key + '.png'), { format: 'png', width: 1024, height: 1280 });
  const sharp = input => {
    let m = Buffer.isBuffer(input) ? JSON.parse(input.toString()) : f.read(input);
    const chain = { metadata: async () => m, png: () => chain, composite: () => chain,
      resize(width, height) { m = { ...m, width, height }; return chain; },
      toBuffer: async () => f.encode(m), toFile: async file => f.mem.writeFileSync(file, f.encode(m)) };
    return chain;
  };
  const donors = [], R = {
    ART: { width: 737, height: 921 }, verifyAssets: async () => f.read(path.join(f.bank, 'manifest.json')),
    previewText: async () => [], composite: async () => f.encode({ format: 'png', width: 897, height: 1497 }),
    command: async () => { throw Error('NO NATIVE COMMANDS IN TESTS'); },
    components: async p => {
      donors.push(structuredClone(p));
      const layer = (name, filename, width, height) => ({ name, input: path.join(f.bank, filename), left: 0, top: 0, width, height });
      return [layer('ILLUSTRATION - cadrage', 'frame.png', 897, 1497), layer('CADRE V4 - structure validee', 'frame.png', 897, 1497),
        layer('FACTION - Chroma', 'none-branch.png', 12, 12), layer('ARME - ' + p.weapon, 'none-branch.png', 12, 12),
        layer('CRISTAL ' + p.element, 'none-crystal.png', 12, 12), layer('BRANCHES ' + p.element + ' - couleur du cristal', 'none-branch.png', 12, 12)];
    }
  };
  f.L.sharp = sharp; f.L.protectedCheck = async () => {};
  f.L.diff = async (a, b) => ({ changed: digest(f.mem.readFileSync(a)) === digest(f.mem.readFileSync(b)) ? 0 : 1 });
  const builder = createBuilder({ L: f.L, D: f.D, R, home: f.home, buildCatalog, createEngine });
  return { ...f, builder, donors, R };
}
module.exports = { fixture, builderFixture, priorPublications, digest, uuid, NAMES };
