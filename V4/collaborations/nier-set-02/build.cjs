...
function createBuilder(options = {}) {
  ...
  async function locked(action) {
    ...
    try {
      fd = fs.openSync(lock, 'wx'); fs.writeFileSync(fd, JSON.stringify({ id, pid: process.pid, kind: 'nier-set-02-production' }));
      return await action();
    } finally {
  ...
  async function check(key) {
    ...
    guard.dependencies(); await assets.verify(); await game();
    return { cards: cards.length, ids: cards.map(c => c.id), fullDeck: false, arenas: [], presets: [],
      missing: cards.map(c => file('art/' + c.key + '.png')).filter(f => !fs.existsSync(f)) };
  }
  function sources(cards) {
    return ['set.json', 'dependencies.json', 'build.cjs', 'model.cjs', 'assets.cjs', 'preservation.cjs', 'publish.cjs', 'compose.jsx'].map(file).concat(
      ...
      assets.inputs(set()), cards.map(c => file('art/' + c.key + '.png')));
  }
