// The verified typography helper is read-only; all FF10 outputs remain isolated.
#include "../nier-pilot-01/typography.jsx"
function composeFF10(home,root){
    ...
    return K.lifecycle(function(life){
        ...
        var doc=life.track(app.documents.add(897,1497,300,'Kalistar FF10 - '+card.name,NewDocumentMode.RGB,DocumentFill.TRANSPARENT,1,BitsPerChannelType.EIGHT,'sRGB IEC61966-2.1'));
        ...
        doc.info.title=card.name+' - '+card.title;doc.info.caption='Kalistar V4 | FF10 private fan crossover | '+card.id+' | Native text and embedded objects';
        ...
        K.write(folder+'native.json',{photoshop:app.version,width:reopened.width.as('px'),height:reopened.height.as('px'),resolution:reopened.resolution,expected:expected,layers:K.state(reopened,[],''),components:plan.layers,typography:KT.snapshot(reopened)});
    });
}
