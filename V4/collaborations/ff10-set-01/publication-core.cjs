...
const SET = 'ff10-set-01';
const M = require('./model.cjs');
const FILES = ['profile.json', 'card.png', 'card.psd', 'verification.json', 'illustration.png'];
const ARENAS = ['ff10-luca-stadium', 'ff10-zanarkand-ruins'];

function createPublisher(options = {}) {
  const M = options.model || require('./model.cjs');
  const SET = options.setId || 'ff10-set-01', faction = options.faction || 'FF10';
  const ARENAS = options.arenas || ['ff10-luca-stadium', 'ff10-zanarkand-ruins'];
  ...
  async function inspect() {
    ...
    for (const spec of set.cards) {
      ...
      assert.ok(p && typeof p.id === 'string' && /^4\d{7}$/.test(p.id) && !ids.has(p.id), 'Identifiants V4 uniques requis.');
      ids.add(p.id);
      M.validateProfile(p, spec);
      assert.ok(p.faction === faction && p.collaboration === faction && p.characterId === M.characterId(spec), 'Identite de collaboration invalide : ' + spec.key);
      ...
      if (p.element === 'NONE') {
        assert.equal(v.none?.unlit, true, 'Preuve NONE eteint absente.');
        const bank = path.join(ROOT, 'V4/atelier/designer-assets'), manifest = read(path.join(bank, 'manifest.json'));
        for (const kind of ['crystal', 'branch']) {
          const asset = L.inside(bank, manifest.elements.NONE[kind].file), expected = await hash(asset);
          assert.equal(v.none[kind + 'Hash'], expected, 'Composant NONE different.');
          observed.set(asset, expected);
        }
      }
      ...
      if (prior) {
        ...
        assert.ok(uuid(prior.creationJob) && old.job === prior.creationJob && old.setId === SET && old.key === spec.key, 'Provenance de creation differente.');
        creation = old;
    ...
    const data = await buildCatalog({ published: next.cards.filter(c => c.kind === 'created') });
    const deck = cards.map(c => c.entry.id);
    const presets = M.validateGame(data, set, createEngine);
    for (const id of ARENAS) {
      const arena = data.arenas.find(a => a.id === id);
      assert.ok(arena && arena.image === '/jeu/assets/arenes/' + id + '.png', 'Arene FF10 absente du catalogue : ' + id);
    ...
    return { before, next, cards, observed, referenceId, deck, presets };
  }
  const report = (plan, mode) => ({ mode, setId: SET, ids: plan.deck, added: plan.cards.filter(c => !c.prior).length,
    reused: plan.cards.filter(c => c.prior).length, presets: plan.presets, arenas: ARENAS });
  ...
  async function publish() {
    ...
    try {
      ...
      const changed = plan.cards.some(c => !c.prior);
      ...
      for (const c of plan.cards) await compareFiles(c.target, c.hashes, FILES);
      await recheck(plan);
