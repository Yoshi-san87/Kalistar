...
var home=File($.fileName).parent.fsName.replace(/\\/g,'/')+'/',request=K.read(home+'solaria-request.json');
...
function embedded(doc,names){
    ...
    for(var i=0;i<names.length;i++){
        ...
        if(layer.name==='OMBRE - CONTENU'){
            if(layer.kind!==LayerKind.NORMAL)throw Error('Original Solaria shadow must remain a raster layer');
            out[layer.name]=false;continue;
        }
...
function revise(item){
    if(item.kind!=='flag')throw Error('Solaria-only native adapter');
...
for(var i=0;i<request.items.length;i++){
    K.write(home+'solaria-progress.json',{index:i,total:request.items.length,key:request.items[i].key});revise(request.items[i]);
}
