...
function nativeTexts(doc){
    ...
    for(var i=0;i<states.length;i++)if(states[i].kind==='LayerKind.TEXT'){
        var l=K.need(doc,states[i].name),t=l.textItem;
        out.push({path:states[i].path,font:t.font,sizePt:t.size.as('pt'),tracking:t.tracking,
            capitalization:String(t.capitalization),fauxBold:t.fauxBold,fauxItalic:t.fauxItalic});
    }return out;
}
