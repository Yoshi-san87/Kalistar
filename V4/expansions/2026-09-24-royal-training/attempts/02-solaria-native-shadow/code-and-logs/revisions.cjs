...
const CODE = ['revisions.cjs', 'revision.jsx', 'geometry.cjs', 'model.cjs', 'scope.cjs', 'preservation.cjs'];
function select(keys) {
  const all = L.read(G.local('baseline/revisions.json')), wanted = keys?.split(',');
  const items = all.filter(c => !wanted || wanted.includes(c.key)); assert.ok(items.length);
...
async function render(keys, { nativeAuthorized = false } = {}) {
  ...
  return G.locked(async () => {
    ...
    L.write(G.local('revision-request.json'), { items: plans });
    const output = await R.command('powershell.exe', ['-NoProfile', '-NonInteractive', '-ExecutionPolicy', 'RemoteSigned', '-File',
      path.join(L.ROOT, 'V4/revisions/2026-09-18-branches/bridge.ps1'), '-Script', G.local('revision.jsx')], G.local('revision-photoshop.log'));
...
async function verify(keys) {
  return G.locked(async () => {
    ...
    for (const item of select(keys)) {
      ...
      for (const layer of plan.layers) assert.equal(n.embedded[layer], true);
