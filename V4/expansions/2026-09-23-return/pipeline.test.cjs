...
function historicalCatalogue() {
  const references = L.read(path.join(__dirname, 'references-before.json'));
  const snapshot = L.read(path.join(__dirname, 'existing-created.snapshot.json'));
  assert.equal(snapshot.referenceId, references.id);
  return { references, published: snapshot.entries };
}
function cards() {
...
test('prospective catalogue executes unchanged builder without writing real registry', async () => {
  ...
  const { references, published } = historicalCatalogue(), originalCount = references.cards.length + published.length;
  ...
  assert.equal(data.cards.length, originalCount + 22);
});

test('historical catalogue fixture still rejects duplicate card identities', async () => {
  const { references, published } = historicalCatalogue(); assert.ok(published.length);
  await assert.rejects(require('./catalogue.cjs').prospective(L, references, [...published, published[0]]), /Identifiants de cartes dupliqu\u00e9s/);
});

test('test-only fixture changes are absent from every native preparation source', () => {
  const builder = require('./build.cjs').createBuilder(), relative = 'V4/expansions/2026-09-23-return/pipeline.test.cjs';
  for (const card of L.read(path.join(__dirname, 'set.json')).cards) {
    assert.ok(!builder.sources(card).some(file => path.resolve(file) === __filename));
    assert.ok(!Object.hasOwn(L.read(path.join(__dirname, 'cards', card.key, 'preparation.json')).inputs, relative));
  }
});
...
async function publicationFixture(t) {
  ...
  const prospective = async (_L, ref, published) => {
    const historical = historicalCatalogue();
    return require('./catalogue.cjs').prospective(L, { ...historical.references, cards: [...historical.references.cards, ...ref.cards] }, [...historical.published, ...published]);
  };
...
test('native accent glyph calibration accepts correct type and rejects font or geometry drift', () => {
  const T = require('./typography.cjs'), calibration = T.loadCalibration();
  const native = L.read(path.join(__dirname, 'attempts/03-before-native-glyph-validation/cards/kaine/render/native.json'));
  assert.equal(native.layers.find(l => l.name === 'NOM').ink[3] - native.layers.find(l => l.name === 'NOM').ink[1], 39);
  T.verify(native, calibration);
  for (const mutate of [n => n.typography.NOM.font = 'ArialMT', n => n.typography.NOM.sizePt = 11,
    n => n.layers.find(l => l.name === 'NOM').ink[3] += 3,
    n => { const l = n.layers.find(l => l.name === 'NOM'); l.ink[1] += 10; l.ink[3] += 10; }]) {
    const bad = structuredClone(native); mutate(bad); assert.throws(() => T.verify(bad, calibration));
  }
});
