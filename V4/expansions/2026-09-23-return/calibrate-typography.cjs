...
async function main() {
  ...
  const names = [...new Set(L.read(path.join(home, 'set.json')).cards.map(c => c.name))].sort();
  const source = L.baseline().cards.find(c => c.key === 'ruby').psd;
  const files = [source, 'V4/collaborations/nier-pilot-01/typography.jsx', 'V4/scripts/stable/common.jsx',
    'V4/expansions/2026-09-23-return/calibrate-typography.jsx'];
  ...
  await require('./build.cjs').createBuilder().locked(async () => {
    ...
    console.log(JSON.stringify({ calibrated: names.length, names: native.names.map(n => ({ name: n.name, width: n.width, height: n.height })) }));
  });
}
