...
const CODE = ['assemble.cjs', 'model.cjs', 'assets.cjs', 'preservation.cjs', 'capture.cjs', 'catalogue.cjs', 'build.cjs', 'compose.jsx', 'canonical.cjs', 'regression.cjs', 'regression.jsx', 'evidence.cjs', 'publish.cjs'];
function createBuilder(options = {}) {
  const L = options.L || require('../../atelier/lib.cjs'), D = options.D || require('../../atelier/designer-core.cjs');
  const R = options.R || require('../../atelier/designer-render.cjs'), T = options.typography || require('../../collaborations/nier-pilot-01/typography.cjs');
  ...
  function sources(c) {
    ...
    return [...CODE, 'set.json', 'dependencies.json', ...Object.keys(M.INPUTS)].map(file).concat(
