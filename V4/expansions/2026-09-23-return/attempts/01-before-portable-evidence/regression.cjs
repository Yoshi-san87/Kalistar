...
async function run() {
  ...
  assert.equal(status.state, 'verified', 'Regression des anciennes references echouee.');
  const oldReportFile = path.join(L.jobDir(job), 'verification.json'), oldReport = read(oldReportFile);
  ...
  return builder.locked(async () => {
    ...
    const report = { passed: true, referenceId: plan.next.id, previousReferenceId: plan.before.id, rendererHash, planDigest: plan.digest,
      oldReportFile: path.relative(L.ROOT, oldReportFile).replace(/\\/g, '/'), oldReportHash: await L.hash(oldReportFile),
