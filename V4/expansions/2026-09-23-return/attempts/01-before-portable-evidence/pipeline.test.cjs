...
async function publicationFixture(t) {
  ...
  const oldReportFile = 'V4/old-native-report.json'; f.L.write(path.join(f.root, oldReportFile), { passed: true });
  f.L.write(file('canonical-regression.json'), { passed: true, planDigest: plan.digest, referenceId: plan.next.id, previousReferenceId: original.id,
    rendererHash: 'fixture-renderer', oldReportFile, oldReportHash: await f.L.hash(path.join(f.root, oldReportFile)), checkedAt: '2026-09-23T00:00:00.000Z',
    results: plan.newEntries.map(e => ({ key: e.key, passed: true, comparison: { changed: 0 }, roundtrip: { changed: 0 }, barcode: { passed: true } })) });
