...
async function main(){
  ...
  await page.locator('[data-view=decks]').click();await page.setViewportSize({width:412,height:1007});
  assert.equal(await page.locator('.decks-preset-tools').count(),0);
  assert.equal(await page.locator('.kdb-demo-tools #deck-preset').count(),1);
  assert.equal(await page.locator('.kdb-demo-tools [data-action]').count(),3);
  const rail=page.locator('.kdb-candidates');
  assert.ok(await rail.locator('.kdb-candidate').count()>8,'all candidates are in one horizontal rail');
  assert.ok(await rail.evaluate(n=>n.scrollWidth>n.clientWidth),'candidate rail overflows horizontally');
  await page.locator('[data-deck-action=rail-right]').click();
  await page.waitForFunction(()=>document.querySelector('.kdb-candidates').scrollLeft>0);
  await page.waitForFunction(()=>!document.querySelector('[data-deck-action=rail-left]').disabled);
  await page.locator('[data-deck-action=rail-left]').click();
  await page.waitForFunction(()=>document.querySelector('.kdb-candidates').scrollLeft===0);
  await page.waitForFunction(()=>document.querySelector('[data-deck-action=rail-left]').disabled);
  await rail.focus();await page.keyboard.press('End');await page.waitForFunction(()=>document.querySelector('.kdb-candidates').scrollLeft>100);
  await page.keyboard.press('Home');await page.waitForFunction(()=>document.querySelector('.kdb-candidates').scrollLeft===0);
  const railBox=await rail.boundingBox();await page.mouse.move(railBox.x+railBox.width/2,railBox.y+railBox.height/2);await page.mouse.wheel(0,180);
  await page.waitForFunction(()=>document.querySelector('.kdb-candidates').scrollLeft>0);
  await page.keyboard.press('Home');await page.waitForFunction(()=>document.querySelector('.kdb-candidates').scrollLeft===0);
  await page.locator('#deck-preset').selectOption('player');await page.locator('[data-action=load-preset]').click();
  const deckExport=page.waitForEvent('download');await page.locator('[data-action=export-deck]').click();assert.equal((await deckExport).suggestedFilename(),'Kalistar-V4-deck.json');
  const deckImport=page.waitForEvent('filechooser');await page.locator('[data-action=import-deck]').click();assert.ok(await deckImport);
  await screen('full-deck-razr');
  await page.setViewportSize({width:1440,height:1000});await screen('deck-rail-desktop');
  assert.ok(await rail.evaluate(n=>[...n.querySelectorAll('.kdb-candidate-image img')].filter(img=>{const r=img.getBoundingClientRect(),v=n.getBoundingClientRect();return r.right>v.left&&r.left<v.right;}).every(img=>img.complete&&img.naturalWidth>0)),'visible rail cards have loaded artwork');
  assert.ok(await rail.evaluate(n=>n.scrollWidth>n.clientWidth),'desktop candidate rail overflows horizontally');
  ...
  assert.deepEqual(errors,[]);console.log('PASS: filters, horizontal recruitment rail, profile-scoped deck links, comparison, undo/redo, drafts, responsive screenshots.');
  await context.close();
}
