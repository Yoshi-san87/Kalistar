(() => {
  ...
  function render(s,{sort='rating',side='all',profiles=null,arenas=null,tab='lineup',page=0,group='core',award=0,spotlight='rating'}={}){
    ...
    const continuous=matchMedia('(max-width:699px), (max-width:950px) and (max-height:500px)').matches;
    ...
    const paginate=items=>{
      if(continuous)return {items,footer:''};
    ...
    if(tab==='lineup'){
      ...
      content=`<section class="match-palmares golden-palmares" aria-label="Trophées de la rencontre">${!stats.complete||stats.partial?`<p class="golden-provisional">${stats.partial?'Historique partiel : trophées non attribués.':'Classement provisoire · trophées attribués en fin de match'}</p>`:''}${trophies.categories.map((t,index)=>{
        ...
        const trophy=`<div class="golden-object">${trophies.image(t.id)}</div>`;
