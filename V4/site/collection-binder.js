(function(root,factory){
  ...
  function create({data,getOwned=()=>[],getCatalogueChanges=()=>[],getFavorites=()=>new Set(),getCareer=()=>null,onFavorite=()=>{},onRegistry=()=>{},onArchives=()=>{},onHistory=()=>{},artImage=c=>globalThis.KalistarCardMedia.image(c,'art'),profile=''}={}){
    ...
    function pocket(group,index){
      ...
      return `<article class="cb-pocket ${owned?'':'is-unowned'}" data-character="${esc(key(c))}" data-card-id="${c.id}" style="--card-color:#${element(c).color};--pocket-order:${index}">
        ...
        <div class="cb-caption"><span class="cb-copy-count" role="img" aria-label="${owned} exemplaire${owned>1?'s':''}" title="${owned} exemplaire${owned>1?'s':''}">${icon('copy')}<b>${owned}</b></span><div class="cb-caption-title"><h2 class="${c.name.length>8?'is-long-name':''}">${esc(c.name)}</h2>${group.length>1?`<span>${group.length} versions</span>`:''}</div>${button('favorite','star','Favori : '+c.name,`data-id="${c.id}" aria-pressed="${favorite}"`)}</div>
      </article>`;
    }
    ...
    function reader(s){
      ...
      return `<div class="cb-reader" data-motion="${motion}" style="--card-color:#${element(c).color}"><figure class="cb-visual"><div class="cb-media-tools" role="group" aria-label="Visuel de la carte"><button type="button" data-binder-action="media" data-id="card" aria-pressed="${!art}">${icon('credit-card')}Carte</button><button type="button" data-binder-action="media" data-id="art" aria-pressed="${art}">${icon('image')}Illustration</button></div><img class="cb-hero-image ${art?'is-art':''}" src="${art?artImage(c):image(c)}" alt="${esc((art?'Illustration de ':'Carte de ')+c.name+' : '+c.title)}"><figcaption>#${c.id}<a href="${esc(c.pngUrl)}" download title="PNG d’impression" aria-label="Télécharger le PNG d’impression">${icon('download')}</a></figcaption></figure>
        ...
        <div class="cb-versions" aria-label="Versions de ${esc(c.name)}">${versions.map((v,i)=>`<button type="button" data-binder-action="version" data-id="${v.id}" aria-pressed="${v.id===selected}" title="${esc(v.title)}">${crystal(v)}<span>${versions.length>1?'Version '+(i+1):'Édition V4'}</span></button>`).join('')}</div>
