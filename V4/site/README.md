...
Elemental roll presentation (2026-09-22): choosing or changing a card immediately displays its existing elemental crystal in the roll well. Engaging the duel awakens both crystals with element-specific effects. Each participant keeps its aura through its own wind-up, then releases a small colored burst together with the travelling jet. The other participant stays awake, including during the Kalistel decision and across phase remounts or reloads. The painted gem stays fixed, without a flat rotation. One shared 30 fps waiting loop pauses in hidden tabs and is cancelled on navigation or remount. Reduced motion uses a static aura and skips the burst; NONE has no elemental effect. The engine still chooses the face; light travels to the printed ATK/DEF value. Existing Kalistel charges and combat rules are preserved.
...
Phone reinforcements: tapping a vacant replaceable position opens the reserve filtered to compatible cards, with deployment into that position. This works for either human player and never exposes or controls the AI reserve. The desktop selection and general reserve browser are unchanged. `node V4/site/mobile-replacement.browser.test.cjs` checks both players, cancellation and deployment through real engine-generated replacement states.
