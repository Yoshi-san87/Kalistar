...
Elemental roll presentation (2026-09-22): choosing or changing a card immediately displays its existing elemental crystal in the roll well. Engaging the duel keeps both crystals awake until the attack roll: fire tongues, electric arcs, water droplets, wind ribbons, frost shards, leaves, mineral fragments, blood droplets, dark wisps or light rays. The painted gem stays fixed, without a flat rotation. One shared 30 fps loop pauses in hidden tabs and is cancelled on roll, navigation or remount. Reduced motion uses a static aura; NONE has no elemental effect. Reloading an engaged duel restores the waiting aura. The engine still chooses the face; light travels to the printed ATK/DEF value. Existing Kalistel charges and combat rules are preserved.
