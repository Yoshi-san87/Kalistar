(() => {
  ...
  function item(group,{deck,favorites,covers,canAdd,db}){
    ...
    return `<article class="collection-item character-item ${owned?'':'unowned'}" data-character="${esc(key(c))}" data-element="${c.element}" style="--character-color:#${data.elements[c.element]?.color||'93AAA5'}">
      <div class="character-stack">${group.filter(v=>v.id!==c.id).slice(0,2).map((v,i)=>`<img class="stack-behind layer-${i}" src="${KalistarCardMedia.image(v)}" alt="" aria-hidden="true" loading="lazy">`).join('')}
      <button class="card-open" data-action="detail" data-id="${c.id}" aria-label="Consulter ${esc(c.name+' · '+c.title)}"><img src="${KalistarCardMedia.image(c)}" alt="${esc(c.name+' · '+c.title)}" width="797" height="1388" loading="lazy"></button>
  ...
  function careerStatistics(stats){
    const metrics=[['kills','skull','Kills'],['holds','ban','Blocks'],['attack','sword','ATK'],['defense','shield','DEF'],['support','hand-heart','Soutiens'],['debuff','shield-minus','ATK retirée'],['reraises','heart-pulse','Vies sauvées'],['mvp','trophy','MVP']];
  ...
  function career(id,db,selected='all'){
    ...
    return `<section class="career-panel"><div class="career-heading"><h3>${icon('medal')}Carrière</h3><label>Exemplaire<select id="career-instance"><option value="all">Tous · ${instances.length} exemplaire${instances.length>1?'s':''}</option>${instances.map(i=>`<option value="${i.id}" ${selected===i.id?'selected':''}>${i.id}</option>`).join('')}</select></label></div>
      ...
      <div class="career-history"><h4>Dernières rencontres</h4>${s.history.slice(0,6).map(r=>`<button data-action="history-match" data-id="${esc(r.matchId)}"><b class="${r.winner===r.side?'won':'lost'}">${r.winner==='draw'?'N':r.winner===r.side?'V':'D'}</b><span>${esc(r.seed)}<small>J${r.side+1} · ${esc(r.instanceId)} · ${new Date(r.finishedAt).toLocaleDateString('fr-FR')}</small></span><span>${r.kills} K<br>${r.holds} Blocks</span>${icon('chevron-right')}</button>`).join('')||'<p class="muted">Aucune rencontre terminée.</p>'}</div></section>`;
  }
