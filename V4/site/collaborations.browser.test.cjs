...
async function main(){
  ...
  try{
    ...
    const cards=['FF7','FF8','NieR','Replicant','Chroma'].map((faction,index)=>({id:String(49998990+index),characterId:'fixture-'+index,name:'FIXTURE',title:'Fixture',faction,race:'ANDROID',element:'ELECTRO',weapon:'Lance',positions:[5]}));
    ...
    const scopes=await page.locator('.cb-scopes [data-binder-action=scope]').evaluateAll(nodes=>Object.fromEntries(nodes.map(node=>[node.dataset.id,{label:node.innerText.replace(/\s+/g,' ').trim(),count:node.querySelector('b')?.textContent||''}])));
    assert.equal(Object.keys(scopes).length,5);
    assert.match(scopes.kalistar.label,/Kalistar/);assert.equal(scopes.kalistar.count,'1');
    assert.match(scopes['final-fantasy'].label,/Final Fantasy/);assert.equal(scopes['final-fantasy'].count,'2');
    assert.match(scopes.nier.label,/NieR/);assert.equal(scopes.nier.count,'2');
    const reports=[];
    for(const [width,height]of [[320,568],[390,844],[700,900],[1024,768],[1600,1000]]){
      ...
      assert(report.inside&&!report.overlap&&report.boxes.every(b=>!b.overflow),'All five scopes fit: '+JSON.stringify(report));
    }
    fs.writeFileSync(path.join(output,'report.json'),JSON.stringify({passed:true,isolated:true,reports},null,2)+'\n');
    console.log('PASS: Kalistar, Final Fantasy and NieR scopes plus existing tabs/counts fit without overlap or scrolling on 320, 390, 700, 1024 and 1600px.');
    await context.close();
