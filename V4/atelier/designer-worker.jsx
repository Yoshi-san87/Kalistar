...
(function () {
    ...
    var active = K.read(root + 'V4/atelier/data/designer/active.json');
    if (!/^[a-f0-9-]{36}$/.test(active.id)) throw Error('Travail invalide.');
    var folder = root + 'V4/atelier/data/designer/jobs/' + active.id + '/';
    var plan = K.read(folder + 'composition.json'), request = K.read(folder + 'request.json'), card = request.card;
    return K.lifecycle(function (life) {
        var source = life.open(root + plan.textSource);
        if (!source.saved) throw Error('Le PSD de reference contient des modifications non enregistrees.');
        ...
        var blank = doc.activeLayer;
        blank.name = 'FOND - noir';
        ...
        for (var i = 0; i < plan.layers.length; i++) {
            ...
            if (!/^[a-z0-9-]+\.png$/.test(spec.file)) throw Error('Composant invalide.');
            var layer = E.place(doc, folder + spec.file, doc.layers[0], spec.name);
            var b = K.bounds(layer), width = spec.width, height = spec.height;
            layer.resize(width / (b[2] - b[0]) * 100, height / (b[3] - b[1]) * 100, AnchorPosition.TOPLEFT);
        ...
        doc.info.caption = 'Kalistar V4 | Atelier | ' + card.id + ' | Composants valides, objets dynamiques separes et textes natifs | 897 x 1497 px, 300 ppp, sRGB.';
        K.save(doc, folder + 'card.psd', folder + 'card.png');
        var reopened = life.open(folder + 'card.psd');
