...
async function branchRevision(card,report,output){
 const revision=report.branchRevision;if(!revision)return null;
 const catalog=read('V4/template-stable/current-elements.json');
 assert.equal(revision.id,catalog.frameRevision.id,'Branch revision mismatch');
 const audit=read(catalog.frameRevision.audit),directory=path.dirname(catalog.frameRevision.audit);
 const original=read(path.join(directory,'references-before.json'));
 assert.equal(crypto.createHash('sha256').update(JSON.stringify(original.protectedFiles)).digest('hex'),audit.originalReferenceId);
 assert.equal(audit.passed,true);assert.equal(audit.componentHash,revision.componentHash);
 const expected=audit.results.find(r=>r.pngHash===revision.pngHash&&r.psdHash===revision.psdHash);assert.ok(expected,'Missing branch audit');
 const relative=path.relative(root,output).replace(/\\/g,'/'),before=path.join(directory,'originals',relative);
 assert.equal(await hash(before),original.protectedFiles[relative],'Original image changed');
 assert.equal(await hash(relative),revision.pngHash,'Revised image changed');
 assert.equal(await hash('V4/templates/'+card.output+'.psd'),revision.psdHash,'Revised PSD changed');
 const a=await pixels(path.join(root,before)),b=await pixels(output);assert.deepEqual(a.info,b.info);
 ...
 for(let y=0;y<a.info.height;y++)for(let x=0;x<a.info.width;x++){
  ...
  changed++;if(!audit.rectangles.some(r=>x>=r[0]&&y>=r[1]&&x<r[2]&&y<r[3]))outside++;
 }
 assert.deepEqual({changed,outside},expected.comparison);assert.equal(outside,0);
 return {id:revision.id,original:path.join(root,before),comparison:{changed,outside}};
}
(async()=>{
 ...
 for(const entry of manifest.cards){
  ...
  const structuralRevision=await branchRevision(c,report,output);
  if(c.element==='NONE'){
  ...
  if(report.textRefresh){
   // Editorial checks belong to their historical revision, before the audited frame correction.
   const before=await pixels(path.join(root,report.textRefresh.before)),after=await pixels(structuralRevision?structuralRevision.original:output);assert.deepEqual(before.info,after.info);
   let outside=0;
  ...
  const checks={card:c.name,id:c.id,reopened,fixed,statsV3Unchanged:true,nativeTexts:true,flagBounds:flag,artworkUnchanged:c.artworkUnchanged,artworkSource:c.artworkSource,structuralRevision};
