...
const REVISION = '2026-09-21-ff8-irvine-background-final';
const { inherit, parentHome } = require('./inherit.cjs');
...
function createRevision(options = {}) {
  ...
  const artInput = path.resolve(options.art || local('art/irvine.png'));
  ...
  async function check() {
    const missing = [artInput].filter(f => !fs.existsSync(f));
  ...
  async function guard(before, mode = 'before') {
    owned(); assert.ok(!fs.existsSync(path.join(parentHome(L), 'transaction.json')), 'La transaction precedente ne doit pas etre publiee.'); assert.equal(before.revision, REVISION); assert.equal(before.referenceId, L.baseline().id);
  ...
  async function prepare() {
    return locked(async () => {
      assert.ok(!fs.existsSync(local('originals')) && !fs.existsSync(local('before.json')), 'Continuation deja preparee : sauvegardes non remplacables.');
      assert.deepEqual((await check()).missing, [], 'Nouvelle illustration Irvine requise.');
      await stable(); await preflight();
      const before = await inherit({ L, R, home, revision: REVISION, cards: KEYS.map(cardPaths), targets: targets(KEYS.map(cardPaths)), art: artInput });
      await guard(before); await stable(); write(local('before.json'), before);
      return { prepared: ['irvine'], reusedNative: KEYS.filter(key => key !== 'irvine'), productionUnchanged: true };
    });
  }
  async function render() {
    return locked(async () => {
      ...
      const cards = before.cards.filter(c => c.key === 'irvine').map(c => ({ key: c.key, work: c.work,
      ...
      await guard(before); return { rendered: ['irvine'], reusedNative: KEYS.filter(key => key !== 'irvine'), output };
    });
  }
...
async function main(args = process.argv.slice(2), revision) {
  const [action = 'check', art] = args;
  assert.ok(args.length <= (action === 'prepare' ? 2 : 1) && ['check', 'prepare', 'render', 'verify', 'publish', 'rollback'].includes(action), 'Usage: node revise.cjs [check|prepare [irvine.png]|render|verify|publish|rollback]');
  return (revision || createRevision({ art }))[action]();
}
