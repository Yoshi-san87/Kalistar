...
async function prepare(key) {
  ...
  return locked(async () => {
    ...
    for (const spec of select(key)) {
      ...
      await sharp(await R.composite(layers)).composite(await R.previewText({ ...donor, race: p.race })).png().toFile(path.join(dir, 'preview.png'));
      ...
      const inputs = ['set.json', 'build.cjs', 'model.cjs', 'assets.cjs', 'compose.jsx', 'compose-one.jsx', 'race-components.json', 'components/race-ANDROID.png', 'components/weapon-Katana.png', 'weapon-katana.json', 'flag-NieR-packed.png', 'faction.json'].map(file);
