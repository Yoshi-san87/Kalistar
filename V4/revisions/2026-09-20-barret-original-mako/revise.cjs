...
async function main() {
  ...
  if(action==='prepare') {
    ...
    const creation=read(path.join(published,'creation.json'));creation.hashes=await hashes(published);creation.artworkRevision='2026-09-20-barret-original-mako';write(path.join(published,'creation.json'),creation);
