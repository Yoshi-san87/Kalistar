...
function complete(){
  ...
  for(let i=0;i<6000&&s.phase!=='over';i++){
    if(s.phase==='choose')E.lock(s,...E.aiChoice(s));
      ...
      const [grant,choice]={guard:['grantGuard','aiGuardChoice'],heart:['grantReraise','aiReraiseChoice'],potion:['grantPotion','aiPotionChoice'],physical:['grantPhysical','aiPhysicalChoice'],clover:['grantClover','aiCloverChoice']}[s.phase];
...
async function run(){
  ...
  await test('NONE catalogue/detail renders without exception or elemental barrier',async()=>{
    ...
    for(const c of data.cards.filter(c=>c.element==='NONE')){
      ...
      await page.locator(`[data-binder-action="open"][data-id="${c.id}"]`).click();
  ...
  await test('UI guard coexists with clover; saved state and DB stats equal the engine',async()=>{
    ...
    assert.equal(await page.locator(`.slot[data-unit="${recipient.uid}"] .trait-badge`).count(),2);
  ...
  await test('Invalid arena, duplicate buff charges and V2 game imports leave active state intact',async()=>{
    for(const mutate of [s=>s.arenaId='absent-place',s=>s.players[0].board[0].mana=120,s=>s.schema=5]){
