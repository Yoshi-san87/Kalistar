(function (root, factory) {
  ...
  function createModel({ data, engine, registry, userId, validateDeck }) {
    if (!data?.cards?.length || !engine || typeof engine.validateDeck !== 'function' || typeof engine.lineup !== 'function' || typeof engine.synergy !== 'function')
      throw new Error('Catalogue et moteur Kalistar V3 requis.');
    ...
    function evaluate(slots) {
      ...
      if (ids.some(id => !byId.has(id))) errors.push('Carte V3 inconnue.');
  ...
  function create(options = {}) {
    ...
    function normalize(value) {
      ...
      if (cards.some(id => id !== null && (typeof id !== 'string' || !byId.has(id)))) throw new Error('Brouillon incompatible avec les cartes V3 connues.');
    ...
    async function click(event) {
      ...
      try {
        ...
        if (action === 'export') {
          ...
          const link = root.ownerDocument.createElement('a'); link.href = url; link.download = 'Kalistar-V3-decks-' + userId.replace(/[^a-z0-9_-]/gi, '_') + '.json'; link.click();
