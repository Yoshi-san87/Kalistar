(() => {
  ...
  function render(s,{sort='rating',side='all',profiles=null,arenas=null,tab='lineup',page=0,group='core',award=0,spotlight='rating'}={}){
    ...
    const portrait=u=>`<img src="assets/cards/${esc(byId[u.cardId].slug)}.webp" alt="" width="34" height="46">`;
    const artwork=u=>{
      const c=engine.byId[u.cardId],path=(c.art||'assets/illustrations/'+c.slug+'.png').replace(/^V3[\\/]/,'');
      return `<img class="award-art" src="${esc('../'+path)}" alt="${esc('Illustration de '+byId[u.cardId].name)}" draggable="false">`;
    };
