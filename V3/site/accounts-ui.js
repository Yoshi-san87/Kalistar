(() => {
  ...
  function create({db,getUserId,onSwitch,onChanged,modal,toast,download}){
    const registry=db.registry,cards=window.KALISTAR_DATA.cards,byId=Object.fromEntries(cards.map(c=>[c.id,c]));
    ...
    const visual=item=>{const c=byId[item.cardId];return `<figure class="transfer-card"><img src="assets/cards/${c.slug}-full.png" alt="${esc(c.name+' - '+c.title)}" width="797" height="1388"><figcaption><b>${esc(c.name)}</b><span>${esc(c.title)}</span><code>${esc(item.id)}</code></figcaption></figure>`;};
    ...
    function hub(){
      ...
      if(tab==='collection')body=`<div class="registry-section-title"><div><h3>Exemplaires enregistres</h3><p>${owned.length} carte${owned.length>1?'s':''} · ${new Set(owned.map(i=>i.cardId)).size} versions</p></div>${action('compose','Transferer','arrow-right-left',owned.length?'':'disabled')}</div><div class="registry-owned">${owned.map(i=>{const c=byId[i.cardId],pending=transfers.some(t=>t.collectibleId===i.id&&t.status==='pending');return `<article class="registry-owned-row"><img src="assets/cards/${c.slug}.webp" alt="${esc(c.name)}"><div><b>${esc(c.name)}</b><small>${esc(c.title)}</small><code>${esc(i.id)}</code></div>${action('compose','','arrow-up-right',`data-id="${esc(i.id)}" title="Transferer ${esc(c.name)}" aria-label="Transferer ${esc(c.name)}" ${pending?'disabled':''}`,'icon-button')}${pending?'<small class="registry-pending-label">Transfert en attente</small>':''}</article>`;}).join('')||`<div class="registry-empty">${icon('layers-3')}<h3>Collection vide</h3><p>Aucun exemplaire attribue a ce profil.</p></div>`}</div>`;
