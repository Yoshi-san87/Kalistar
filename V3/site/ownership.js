(() => {
  ...
  function seed(s,write,data){
    if(s.registryMeta.some(m=>m.id==='seed-v1'))return;
    if(stores.some(k=>s[k].length))fail('INVALID_REGISTRY','Registre incomplet : restauration explicite requise.');
    const createdAt=now();
    for(const user of profiles)write('users',copy(user));
    for(const card of data.cards){
      const item={id:'KC-'+random(16),cardId:String(card.id),ownerId:PARIS,status:'owned',createdAt,legacyInstanceId:`K3-${card.id}-001`};
      write('collectibles',item);event(s,write,'seeded',item,PARIS);
    }
    write('registryMeta',{id:'seed-v1',version:1,createdAt});
  }
  ...
  function validateBackup(value,data){
    ...
    for(const c of s.collectibles){
      ...
      if(c.legacyInstanceId!==undefined){
        if(c.legacyInstanceId!==`K3-${c.cardId}-001`||legacy.has(c.legacyInstanceId)||c.status!=='owned')fail('INVALID_BACKUP','Liaison historique originale invalide.');
    ...
    for(const e of s.events){
      ...
      if(e.type==='bound'||e.type==='released'){
        if(!/^match-[a-f0-9-]{36}$/.test(e.matchId)||e.transferId!==undefined||!new RegExp('^K3-'+collectible(s,e.collectibleId).cardId+'-00[1-4]$').test(e.instanceId))fail('INVALID_BACKUP','Historique de partie invalide.');
