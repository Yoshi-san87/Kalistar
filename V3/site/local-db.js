(() => {
  ...
  async function open(data,{name=NAME}={}) {
    ...
    const close=()=>{opened=false;channel?.close();connections.get(name)?.delete(refresh);db.close();};
    ...
    const api={
      ...
      importBackup:(input,options={})=>{const value=copy(input),replaceRegistry=options.replaceRegistry===true;return enqueue(async()=>{
        ...
        const registry=value.schema===3?O.validateBackup(value,data):null;
        if(value.versions.length>1000||new Set(value.versions.map(p=>p?.id)).size!==value.versions.length||value.versions.some(p=>!E.byId[p?.id]||p.characterId!==E.byId[p.id].characterId))throw legacyError();
        ...
        await atomic((s,write,clear)=>{
          ...
          if(registry&&!replace&&!O.sameRegistry(s,registry))O.fail('REGISTRY_CONFLICT','Le registre differe de cette sauvegarde. Une restauration complete confirmee est requise.');
          if(replace){
            for(const store of allStores)clear(store);
            for(const store of registryStores)for(const item of registry[store])write(store,item);
          ...
          mergeMatches(matches,instances,s,write,{restoring:!!registry,restoreLegacy:!!replace});
