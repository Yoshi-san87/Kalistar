(async () => {
  ...
  try{const saved=localStorage.getItem('kalistar.v3.activeUser');if(db?.registry?.user(saved))accountId=saved;}catch{storageAvailable=false;}
  window.KALISTAR_ACTIVE_USER=accountId;
  const preferencePrefix=accountId===KalistarOwnership.PARIS?'kalistar.v3.':'kalistar.v3.'+accountId+'.';
  ...
  const ui={view:location.hash==='#arena'?'arena':location.hash==='#decks'?'decks':'collection',attacker:null,target:null,reserve:null,replacementSlot:null,detail:null,art:false};
  ...
  function collectionView(){
    if(!collectionBinder)collectionBinder=KalistarCollection.create({
      data,getOwned:owned,getFavorites:()=>favorites,getCareer:(id,instance)=>collectionDB()?.career(id,instance),artImage,
...
function showDeck(){setView('decks');}
  ...
  function showDetail(id,art=false,context=null){
    ...
    const c=profile?{...profile,id:current.id,slug:current.slug,art:current.art}:current,el=elementInfo(c);
  ...
  document.addEventListener('change',async e=>{
    ...
    if(e.target.id==='library-file'){
      ...
      try{
        ...
        if(value.schema>=3&&!confirm('Restaurer cette sauvegarde complete ? Les proprietaires, activations et transferts locaux seront remplaces. Fermez les autres onglets Kalistar et conservez d’abord une sauvegarde actuelle.'))return;
        await db.importBackup(value,{replaceRegistry:value.schema>=3});await db.registry.refresh();
        if(value.schema>=3){try{for(const prefix of ['kalistar.v3.','kalistar.v3.user-tokyo.'])localStorage.removeItem(prefix+'game');}catch{storageAvailable=false;}suspendGame();}
  ...
  window.addEventListener('hashchange',()=>setView(location.hash==='#arena'?'arena':location.hash==='#decks'?'decks':'collection'));
  ...
  if(db?.registry)accountsUI=KalistarAccountsUI.create({db,getUserId:()=>accountId,onChanged:ownershipChanged,modal,toast,download,onSwitch:async id=>{if(!db.registry.user(id))throw new Error('Profil inconnu.');persist();await db.idle();localStorage.setItem('kalistar.v3.activeUser',id);location.reload();}});
