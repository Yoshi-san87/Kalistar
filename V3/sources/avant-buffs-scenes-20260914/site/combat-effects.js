(() => {
  ...
  async function play({before,after,element,color,reduced,signal}){
    ...
    try{
      if(['guard','guard-gain','heart','heart-gain','mana','mana-gain','power','luck','clover-gain','second-chance'].includes(reaction)){
        const recipient=(['guard-gain','heart-gain','clover-gain','mana-gain'].includes(reaction)?field.querySelector(`.slot[data-unit="${d.guardGranted||d.wardGranted||d.reraiseGranted||d.cloverGranted||d.manaGranted}"]`):before.phase==='attack'?source:target)||source,card=recipient.querySelector('.slot-card');
