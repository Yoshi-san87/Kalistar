(function (root, factory) {
  ...
  function createEngine(data) {
    const validCardId=id=>(typeof id==='string'||Number.isInteger(id))&&/^300000(?:0[1-9]|[1-3][0-9]|4[01])$/.test(String(id));
    ...
    if(!data||!Array.isArray(data.cards)||!data.cards.length||!data.elements||!data.weapons)throw new Error('Données moteur V3 invalides.');
    for(const c of data.cards){
      if(!c||!validCardId(c.id))throw new Error('Carte incompatible V3 : identifiant V3 requis. Les cartes V2 sont refusées.');
      if(typeof c.characterId!=='string'||!c.characterId.trim()||![1,2,3,4,5].includes(c.role)||
        ...
        !Array.isArray(c.positions)||!c.positions.length||c.positions.some(p=>![1,2,3,4,5].includes(p)))throw new Error('Profil V3 invalide : '+c.id);
      ...
      if(!Array.isArray(c.atk)||c.atk.length!==6||c.atk.some(v=>!numericFace(v)&&!['retry','mana','buff_atk','revive','death','guard'].includes(v))||
        !Array.isArray(c.defense)||c.defense.length!==6||c.defense.some(v=>!numericFace(v)&&!['retry','dodge'].includes(v)))throw new Error('Faces V3 invalides : aucun bouclier spécial en DEF.');
      ...
      if(typeof c.element!=='string'||c.element!=='NONE'&&!Object.hasOwn(data.elements,c.element)||
        ...
        c.element==='NONE'&&c.barriers.length)throw new Error('Cristal ou barrière V3 invalide. NONE ne porte pas de barrière.');
    }
    ...
    for(const arena of arenas){
      if(!arena||typeof arena.id!=='string'||!/^[-a-z0-9]+$/.test(arena.id)||arenaById.has(arena.id)||
        ...
        !Array.isArray(arena.homeCharacters)||arena.homeCharacters.some(id=>typeof id!=='string'||!id.trim()))throw new Error('Arène V3 invalide. Bonus maximum : +25 ATK / +10 DEF.');
    ...
    const traitNames={reraise:'Reraise',luck:'Trèfle',mana:'Potion magique',physical:'Puissance physique',ward:'Garde physique'};
    const trait = unit => traitKeys.find(key=>unit?.[key]>0)||null;
    const instanceId=(cardId,copy)=>`K3-${cardId}-${String(copy).padStart(3,'0')}`;
    ...
    function validateDeck(ids) {
      ...
      if(ids.some(id=>!validCardId(id)))errors.push('Identifiants V3 requis : les decks V2 sont refusés.');
    ...
    function newGame(deckA,deckB,options={}) {
      ...
      const s={schema:6,phase:'setup',turn:0,round:1,rng:random||1,seed,mode:options.mode||'ai',players:[],log:[],duel:null,lastDuel:null,winner:null,replacing:null};
    ...
    function recordPerformance(s){
      ...
      s.match.events.push({round:s.round,attacker:d.attacker,target:d.target,magic:!!d.magic,
        ...
        support,recipient:support?(d.cloverGranted||d.manaGranted||d.reraiseGranted||d.guardGranted||d.attacker):null,refresh:!!d.traitRefreshed,
    ...
    function matchStats(s){
      ...
      return {version:3,matchId:s.matchId,seed:s.seed,arenaId:s.arenaId,winner:s.winner,complete:s.phase==='over',
    ...
    function validatePerformance(s){
      ...
      for(const e of m.events){
        ...
        if(![0,60].includes(e.ward)||!Number.isInteger(e.arenaAttack)||e.arenaAttack<0||e.arenaAttack>25||!Number.isInteger(e.arenaDefense)||e.arenaDefense<0||e.arenaDefense>10||e.magic&&e.ward)throw new Error('Bonus V3 du bilan invalide.');
    ...
    function grantGuard(s,uid){
      ...
      return finish(s,`${card(u).name} reçoit une garde : +60 DEF sur sa prochaine défense numérique physique. Aucun cumul.`);
    }
    ...
    function rollAttack(s,forced) {
      ...
      if(c.atk.includes('guard')&&!canGuard(c)||c.atk.includes('revive')&&!canHeal(c))throw new Error('Face guard/Reraise incompatible avec le profil V3.');
    ...
    function assertState(s) {
      ...
      const phases=['setup','choose','attack','defense','clover','potion','heart','guard','result','replace','over'];
      ...
      for(let side=0;side<2;side++){
        ...
        if(all.some(u=>u&&(!validCardId(u.cardId)||typeof u.instanceId==='string'&&u.instanceId.startsWith('K2-'))))throw new Error('Partie V2 incompatible avec la V3 : identifiants K3 et cartes V3 requis.');
        if(all.length!==10||all.some(u=>!u||!byId[u.cardId]||!new RegExp('^'+side+'-[0-9]$').test(u.uid)||typeof u.revived!=='boolean'||![0,60].includes(u.mana)||![0,60].includes(u.physical)||![0,60].includes(u.ward))||new Set(all.map(u=>u.uid)).size!==10||validateDeck(all.map(u=>u.cardId)).length)throw new Error('Cartes de sauvegarde invalides.');
        for(const u of all){
          if(typeof u.entered!=='boolean'||typeof u.instanceId!=='string'||!new RegExp('^K3-'+u.cardId+'-00[1-4]$').test(u.instanceId)||instanceIds.has(u.instanceId))throw new Error('Identité de carte invalide.');
        ...
        if(all.some(u=>traitKeys.filter(key=>u[key]>0).length>1))throw new Error('Un seul trait actif par carte.');
      }
      ...
      for(const d of [s.duel,s.lastDuel])if(d!==null){
        ...
        if(d.attackValue==='guard'&&!canGuard(author)||d.attackValue==='revive'&&!canHeal(author))throw new Error('Soutien incompatible avec le profil V3.');
        for(const f of [d.formula,d.failedDefense])if(f!==undefined){
          ...
          if(!f||fields.some(k=>!Number.isInteger(f[k])||Math.abs(f[k])>2000)||typeof f.magic!=='boolean'||f.magic!==d.magic||
            ...
            f.defense!==Math.max(0,f.baseDefense+f.race+f.arenaDefense+f.ward))throw new Error('Calcul V3 invalide.');
        }
      ...
      if(['attack','defense','clover','potion','heart','guard','result'].includes(s.phase)&&!s.duel)throw new Error('Duel manquant.');
      if(['attack','defense','clover','potion','heart','guard'].includes(s.phase)){
        const d=s.duel;
    ...
    function assertV3(s){
      if(s&&[1,2,3,4,5].includes(s.schema))throw new Error('Partie V2 incompatible avec la V3 : import refusé. Démarrez une nouvelle partie V3 ; vos sauvegardes et statistiques V2 restent séparées.');
      if(!s||s.schema!==6)throw new Error('Sauvegarde incompatible : schéma 6 V3 requis.');
    }
    function restoreGame(value) {
      assertState(value);
      return clone(value);
    }
    return {data,rules,byId,card,trait,clone,dieValue,mean,lineup,validateDeck,newGame,deploy,recall,autoDeploy,start,synergy,elementModifier,lock,rollAttack,rollDefense,grantClover,grantPotion,grantReraise,grantGuard,aiCloverChoice,aiPotionChoice,aiReraiseChoice,aiGuardChoice,arenaBonuses,setArena,next,aiChoice,assertState,restoreGame,matchStats,instanceId};
  }
