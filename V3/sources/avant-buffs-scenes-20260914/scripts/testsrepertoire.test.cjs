...
test('V2-shaped profiles, required narrative metadata and local V3 destinations',()=>{
  for(const c of cards) {
    ...
    assert.match(c.prompt,/deux bras/);
    assert.match(c.prompt,/deux jambes/);
  }
...
test('Production manifest: 14 true V2 copy sources, two redraws, 25 new images',()=>{
  const assignments=registry.imageAssignments;
  assert.equal(assignments.length,41);
  assert.deepEqual(assignments.reduce((a,x)=>(a[x.action]=(a[x.action]||0)+1,a),{}),{copy:14,redraw:2,generate:25});
  ...
  assert.deepEqual(cards.filter(c=>c.art_action==='redraw').map(c=>c.name),['JULIENNE','KAYLIS']);
