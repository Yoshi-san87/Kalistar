...
const files = ['app.js', 'local-db.js', 'catalogue.js', 'engine.js', 'data.js', 'v3.css'];
...
for (const preset of presets) assert.deepEqual(E.validateDeck(preset.cards), [], preset.id);
...
async function run() {
  ...
  await test('Five named presets cover 41 cards and all pass validateDeck', async () => {
  ...
  await test('Editing the initial draft cannot mutate source presets', async () => {
    await page.locator(`#deck-dialog [data-action="remove"][data-id="${data.decks.player[0]}"]`).click();
  ...
  const running = await saved(page, 'game'); assert.equal(running.phase, 'choose');
  ...
  await test('Selecting is inert; Charger loads each draft without changing the active match', async () => {
    ...
    for (const preset of presets) {
      ...
      assert.equal(await page.locator('#deck-name').inputValue(), preset.name);
      assert.equal(await page.locator('[data-action="deck-play"]').isEnabled(), true);
      assert.deepEqual(await saved(page, 'game'), running);
  ...
  await test('Draft and preset preferences persist in V3; canceled IA choice preserves the match', async () => {
    await close(page); await page.locator('[data-action="new-game"]').click();
  ...
  await test('Opponent selector remains coherent for local two-player matches', async () => {
    ...
    await page.locator('#new-game-form button[type="submit"]').click();
    const s = await saved(page, 'game'); assert.equal(s.mode, 'local');
  ...
  await test('An invalid draft cannot replace an active game, even by forged form submission', async () => {
    ...
    await page.locator('[data-action="deck"]').click();
    await page.locator(`#deck-dialog [data-action="remove"][data-id="${presets.at(-1).cards[0]}"]`).click();
    assert.equal(await page.locator('[data-action="deck-play"]').isDisabled(), true);
    await close(page); await page.locator('[data-action="new-game"]').click();
  ...
  await test('Preset controls fit desktop and mobile dialogs without horizontal overflow', async () => {
    for (const width of [1280, 360]) {
      ...
      for (const selector of ['#deck-preset', '[data-action="load-preset"]']) {
        ...
        assert.equal(await control.evaluate(node => {
          ...
          return box.width > 0 && box.left >= dialog.left && box.right <= dialog.right && box.right <= innerWidth;
  ...
  console.log(JSON.stringify({ ok: true, checks: cases.length, decks: presets.length, cards: 41, browserErrors, changedDuringTest: changed, sourceHashes: Object.fromEntries(files.map(f => [f, createHash('sha256').update(sources[f]).digest('hex').slice(0, 12)])) }, null, 2));
}
