...
async function main() {
  ...
  fs.writeFileSync(path.join(root,'verification','game-bundle.json'),JSON.stringify({cards:cards.length,exported:cards.length-missing.length,missing,crop},null,2));
  console.log('V3 data bundle: '+cards.length+' profiles; '+(cards.length-missing.length)+' cropped card images.');
