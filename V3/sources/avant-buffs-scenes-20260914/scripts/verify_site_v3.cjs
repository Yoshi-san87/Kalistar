...
function deckFor(card) {
  ...
  for (let role = 1; role <= 5; role++) {
    if (!ids.some(id => E.byId[id].positions.includes(role))) ids.push(data.cards.find(c => c.positions.includes(role) && !ids.includes(c.id) && c.element !== 'RAINBOW').id);
  }
  for (const c of data.cards) if (ids.length < 10 && !ids.includes(c.id) && c.element !== 'RAINBOW') ids.push(c.id);
  assert.deepEqual(E.validateDeck(ids), []);
  return ids;
}
function scenario(kind, side = 0) {
  ...
  for (const a of attacks.filter(Boolean)) for (let seed = 0; seed < 200; seed++) {
    ...
    const s = E.newGame(...ds, { mode: 'local', seed: 'UI-V3-' + seed, arenaId: arena.id });
...
function completedMatch() {
  const s = E.newGame(data.decks.player, data.decks.enemy, { mode: 'local', seed: 'V3-ARCHIVES', arenaId: data.arenas[0].id });
...
async function run() {
  ...
  assert.equal(await page.locator('.version-count').count(), 1);
  assert.match(await page.locator('.character-item:has(.version-count) h2').innerText(), /MOMO/i);
  ...
  await page.locator(`[data-action="detail"][data-id="${none.id}"]`).first().click();
  ...
  assert.equal(await page.locator('.detail-visual>img').getAttribute('src'), `assets/cards/${none.slug}-full.png`);
  assert.equal(await page.locator('.detail-actions a[download]').getAttribute('href'), `../cartes/${none.slug}.png`);
  await page.locator('[data-action="toggle-art"]').click();
  assert.equal(await page.locator('.detail-visual>img').getAttribute('src'), `../${(none.art || 'assets/illustrations/' + none.slug + '.png').replace(/^V3[\\/]/, '')}`);
  await closeDialogs();
  ...
  for (const self of [false, true]) {
    ...
    assert.equal(recipient.ward, 60); assert.equal(recipient.luck, 0);
  ...
  report.tests.push('Guard allié/auteur/IA ; ward 60 remplace le trait ; dés fixes');
  ...
  for (const c of data.cards.filter(c => c.characterId === data.cards.find(x => /MOMO/i.test(x.name)).characterId)) {
    ...
    const s = E.newGame(deckFor(c), deckFor(none), { mode: 'local', seed: 'MOMO-HOME', arenaId: arena.id });
  ...
  await page.evaluate(async () => {
    ...
    try {
      ...
      const restored = (await dest.exportBackup()).matches[0];
      ...
      for (const arenas of invalid) {
        const bad = structuredClone(backup); bad.matches[0].arenas = arenas;
  ...
  await fallback.locator(`[data-action="detail"][data-id="${none.id}"]`).first().click();
  assert.match(await fallback.locator('.affinities').innerText(), /sans cristal/i);
  await fallback.locator('[data-action="toggle-art"]').click();
  assert.equal(await fallback.locator('.detail-visual>img').getAttribute('src'), '../assets/illustrations/' + none.slug + '.png');
  ...
  if (!mocked) {
    ...
    assert.equal(await local.locator('.character-item').count(), 40);
