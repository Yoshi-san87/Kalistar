...
pdf.setTitle('Kalistar V1 - 12 cartes - format source 300 ppp')
...
pdf_path=ROOT/'impression/KALISTAR_V1_12_CARTES.pdf'
...
draw.text((30,20),'KALISTAR / V1',font=font,fill='white')
...
page='''<!doctype html><html lang="fr"><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Kalistar V1</title>
...
<header><h1>KALISTAR <small>V1</small></h1><a href="impression/KALISTAR_V1_12_CARTES.pdf">PDF taille réelle</a><a href="KALISTAR_V1_CARTES_PNG.zip">Les 12 cartes PNG</a><select aria-label="Élément"><option value="">Tous les éléments</option>'''+''.join('<option>'+x['id']+'</option>' for x in elements.values())+'''</select></header><main>'''+''.join(items)+'''</main><footer>897 × 1497 px · 300 ppp · Prototype V1</footer><script>document.querySelector('select').addEventListener('change',e=>document.querySelectorAll('figure').forEach(f=>f.hidden=!!e.target.value&&f.dataset.element!==e.target.value))</script></html>'''
(ROOT/'OUVRIR_LES_CARTES.html').write_text(page,encoding='utf8')
with zipfile.ZipFile(ROOT/'KALISTAR_V1_CARTES_PNG.zip','w',zipfile.ZIP_DEFLATED) as z:
