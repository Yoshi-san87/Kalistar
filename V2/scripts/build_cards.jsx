#target photoshop
(function () {
 var ROOT='C:/Users/guill/Documents/Doc/GP-2 inside/Cartes/Kalistar/V2/';
 ...
 function build(c){
  ...
  var art=place(c.reference||c.art,'ILLUSTRATION - remplacer le contenu',c.reference?[0,0,897,1497]:[80,160,817,1073],null,map[873],'cover');
  var weapon=place(ROOT+'assets/armes/'+('0'+c.weapon_index).slice(-2)+'.png','ARME - '+c.weapon,[113,1126,169,1182],dynamic);
  ...
  var race=place(ROOT+'assets/races/'+c.race+'.png','PICTOGRAMME RACE - '+c.race,[730,1129,782,1181],dynamic,null,'contain');
  ...
  contents(128,c.element=='RAINBOW'?'+40 CLASSIQUES':'+'+c.advantage+' '+elements[el.strong_against].label,'AVANTAGE ELEMENTAIRE',240);
  contents(124,c.element=='RAINBOW'?'NEUTRE RAINBOW':'-'+c.disadvantage+' '+elements[el.weak_against].label,'FAIBLESSE ELEMENTAIRE',240);
  hide(871);hide(867);
  ...
  for(j=0;j<6;j++){
   ...
   if(has(c.magic,die)){
    ...
    halo(ROOT+'assets/effets/'+c.element+'.png','ATK D'+die+' - HALO MAGIQUE',geometry.halos[c.element],cx,cy,radius,l);
   }
 ...
 try {
  for(var i=0;i<cards.length;i++)if(config.ids?has(config.ids,cards[i].id):(config.names.length==0||has(config.names,cards[i].name)))build(cards[i]);
