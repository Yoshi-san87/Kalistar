(() => {
  ...
  function dice(side){
    const d=['attack','defense','result'].includes(game.phase)?game.duel:null,isAttack=d?d.side===side:game.turn===side,rolls=d?(isAttack?d.attackRolls:d.defenseRolls):[],die=rolls.at(-1);
  ...
  function duelRecap(s){
    const d=['attack','defense','result'].includes(s.phase)?s.duel:null;
    ...
    if(!numeric){
      const label=d.attackValue==='death'?'Mort · ignore les scores':format(d.attackValue);
      return `<div class="duel-recap special-recap"><span>${esc(label)}</span>${d.defenseRolls.length?`<span>DEF · ${esc(format(d.defenseValue))}</span>`:''}</div>`;
    }
  ...
  function consoleBody(s=game){
    ...
    if(s.phase==='setup'){
      ...
      label=`${atk?'ATTAQUE':'DÉFENSE'} · JOUEUR ${actor+1}`;title=retry?'Nouveau jet':atk?'L’attaque se prépare':'La défense se prépare';
      actions=`<button class="primary" data-action="roll" ${auto||rolling?'disabled':''}>${icon('dice-6')}${auto?'Jet adverse…':retry?'Relancer le dé':atk?'Lancer l’attaque':'Lancer la défense'}</button>`;
  ...
  async function animatedRoll(){
    ...
    try{
      ...
      if(token===epoch&&landed){
        ...
        if(label)label.textContent='D'+value+' · '+format(phase==='attack'?next.duel.attackValue:next.duel.defenseValue);
