(() => {
  ...
  function consoleParticipants(s){
    const d=['attack','defense','clover','result'].includes(s.phase)?s.duel:null,side=d?.side??s.turn;
  ...
  function syncConsole(s=game){
    ...
    node.style.setProperty('--action-color',s.phase==='clover'||s.duel?.autoDefense?'#9bebac':s.phase==='defense'?color(bc)||'#9acdd5':color(ac)||'#d3b7ab');
