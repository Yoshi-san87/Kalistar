(() => {
  ...
  async function play({before,after,element,color,reduced,signal}){
    ...
    try{
      if(['heart-gain','mana','mana-gain','power','luck','clover-gain','second-chance'].includes(reaction)){
        const recipient=['clover-gain','mana-gain'].includes(reaction)?field.querySelector(`.slot[data-unit="${d.cloverGranted||d.manaGranted}"]`):before.phase==='attack'?source:target,card=recipient.querySelector('.slot-card');
        field.dataset.combat='support';field.dataset.reaction=reaction;recipient.classList.add('effect-recipient');
        const [asset,label,tint]=reaction==='heart-gain'?['revive','Reraise','#ff96b7']:reaction==='mana-gain'?['mana','Magie +60','#74d9f7']:reaction==='mana'?['mana','Potion obtenue','#74d9f7']:reaction==='power'?['buff_atk','Physique +60','#f49b78']:['retry',reaction==='clover-gain'?'Trèfle':reaction==='second-chance'?'Seconde chance':before.phase==='attack'?'Trèfle obtenu':'Relance','#7eeb9b'];
