(function (root, factory) {
  ...
  function createEngine(data) {
    ...
    function newGame(deckA,deckB,options={}) {
      ...
      const s={schema:4,phase:'setup',turn:0,round:1,rng:random||1,seed,mode:options.mode||'ai',players:[],log:[],duel:null,lastDuel:null,winner:null,replacing:null};
    ...
    function recordPerformance(s){
      ...
      s.match.events.push({round:s.round,attacker:d.attacker,target:d.target,magic:!!d.magic,
        ...
        support,recipient:support?(d.cloverGranted||d.manaGranted||d.attacker):null,refresh:!!d.traitRefreshed,
        attackRolls:d.attackRolls.length,defenseRolls:d.defenseRolls.length});
    }
    function matchStats(s){
      const metrics=['attack','defense','breakthrough','kills','deaths','holds','support','alliedSupport','refreshes','hearts','clovers','potions','physical','reraises','luckUsed','dodges','shields','barrier','buff','duels','defended','attackRolls','defenseRolls','peakAttack','peakDefense','rating'];
      const empty=()=>Object.fromEntries(metrics.map(k=>[k,0]));
      const units=s.players.flatMap((p,side)=>[...p.board.filter(Boolean),...p.reserve,...p.dead].map(u=>({uid:u.uid,cardId:u.cardId,side,...empty()})));
      ...
      for(const e of s.match?.events||[]){
        ...
        if(!e.support){b.defended++;b.defenseRolls+=e.defenseRolls;b.defense+=e.defense;b.peakDefense=Math.max(b.peakDefense,e.defense);b.holds+=Number(e.hold);b.dodges+=Number(e.dodge);b.shields+=Number(e.shield);b.reraises+=Number(e.reraise);b.luckUsed+=Number(e.luck);b.barrier+=e.barrier;}
        if(e.support){
      ...
      for(const u of units)u.rating=u.kills*5+u.holds*3+u.support*2+u.reraises;
      ...
      return {version:1,seed:s.seed,arenaId:s.arenaId||'ruins',winner:s.winner,complete:s.phase==='over',
    ...
    function grantReraise(s) {
      const d=s.duel,u=s.players[d.side].board[d.attackerSlot],already=setTrait(s,u,'reraise');
    ...
    function assertState(s) {
      if(!s||![1,2,3,4].includes(s.schema)||!Array.isArray(s.players)||s.players.length!==2)throw new Error('Sauvegarde incompatible.');
      validatePerformance(s);
      const phases=['setup','choose','attack','defense','clover','potion','revive','result','replace','over'];
      ...
      if(['attack','defense','clover','potion','revive','result'].includes(s.phase)&&!s.duel)throw new Error('Duel manquant.');
      if(['attack','defense','clover','potion','revive'].includes(s.phase)){
        const d=s.duel;
    ...
    return {data,rules,byId,card,trait,clone,dieValue,mean,lineup,validateDeck,newGame,deploy,recall,autoDeploy,start,synergy,elementModifier,lock,rollAttack,rollDefense,grantClover,grantPotion,aiCloverChoice,aiPotionChoice,next,aiChoice,assertState,restoreGame,matchStats};
  }
