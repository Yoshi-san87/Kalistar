(() => {
  ...
  function render(){
    ...
    persist();scheduleAI();
    if(ui.view==='arena'&&game?.phase==='over'&&!ui.endShown){ui.endShown=true;showMatchStats();}
  }
  ...
  function showMatchStats(){if(game)modal('match-dialog',head(game.phase==='over'?'Palmarès de la rencontre':'Statistiques du match')+KalistarMatchReport.render(game,{sort:statsSort,side:statsSide}));}
  ...
  function board(side){
    ...
    return `<div class="formation cross-formation ${side?'right-cross':'left-cross'}" data-player="${side}">${p.board.map((u,slot)=>{
      ...
      const beneficiary=u&&['clover','potion'].includes(game.phase)&&game.turn===side&&!(game.mode==='ai'&&side===1);
      const currentTrait=E.trait(u),grantName=game.phase==='potion'?'la potion magique':'le trèfle';
  ...
  function dice(side){
    const d=['attack','defense','clover','potion','result'].includes(game.phase)?game.duel:null,isAttack=d?d.side===side:game.turn===side,rolls=d?(isAttack?d.attackRolls:d.defenseRolls):[],die=rolls.at(-1);
  ...
  function consoleParticipants(s){
    const d=['attack','defense','clover','potion','result'].includes(s.phase)?s.duel:null,side=d?.side??s.turn;
  ...
  function syncConsole(s=game){
    ...
    node.style.setProperty('--action-color',s.phase==='potion'?'#8adbf5':s.phase==='clover'||s.duel?.autoDefense?'#9bebac':s.phase==='defense'?color(bc)||'#9acdd5':color(ac)||'#d3b7ab');
  ...
  function duelRecap(s){
    const d=['attack','defense','clover','potion','result'].includes(s.phase)?s.duel:null;
    ...
    if(!numeric){
      const support=['retry','mana'].includes(d.attackValue),label=d.attackValue==='death'?'Mort · ignore les scores':s.phase==='clover'?'Trèfle à attribuer':s.phase==='potion'?'Potion magique à attribuer':dieLabel(d.attackValue,true);
      return `<div class="duel-recap special-recap">${support?`<img class="recap-clover" src="${asset('effets',d.attackValue)}" alt="">`:''}<span>${esc(label)}</span>${support?`<span>${d.attackValue==='mana'?'+60 à la prochaine attaque magique':'Seconde chance en défense'}</span><span class="single-trait-note">Un seul trait par carte.<br>Le nouveau remplace l’ancien.</span>`:''}${d.defenseRolls.length?`<span>DEF · ${esc(format(d.defenseValue))}</span>`:''}</div>`;
    }
  ...
  function consoleBody(s=game){
    ...
    }else if(s.phase==='clover'||s.phase==='potion'){
      const potion=s.phase==='potion';label=`${potion?'POTION MAGIQUE':'TRÈFLE'} · JOUEUR ${s.turn+1}`;title=s.mode==='ai'&&s.turn===1?'Le Veilleur choisit un allié':'Choisir une carte alliée';
      actions=`<span class="clover-choice-status">${icon(potion?'flask-conical':'clover')}${potion?'Attribution de la potion':'Attribution du trèfle'}</span>`;
  ...
  async function animatedTrait(uid){
    if(rolling||!['clover','potion'].includes(game.phase))return;
    const token=epoch,next=E.clone(game);
    try{
      if(game.phase==='potion')E.grantPotion(next,uid);else E.grantClover(next,uid);
  ...
  function scheduleAI(){
    ...
    const active=second||game.mode==='ai'&&((p==='choose'&&game.turn===1)||(['clover','potion'].includes(p)&&game.turn===1)||(p==='attack'&&game.turn===1)||(p==='defense'&&game.turn===0)||(p==='replace'&&game.replacing===1));
    if(!active)return;
    aiTimer=setTimeout(()=>{if(token!==epoch||ui.view!=='arena')return;
      if(p==='attack'||p==='defense')return animatedRoll();
      if(p==='clover'||p==='potion')return animatedTrait(p==='potion'?E.aiPotionChoice(game):E.aiCloverChoice(game));
  ...
  document.addEventListener('click',event=>{
    ...
    try{
      ...
      if(action==='match-stats')return showMatchStats();
      if(action==='stats-sort'){statsSort=id;return showMatchStats();}
      if(action==='stats-side'){statsSide=id;return showMatchStats();}
      if(action==='export-stats')return download('Kalistar-bilan-'+game.seed.replace(/[^a-zA-Z0-9_-]/g,'')+'.json',E.matchStats(game));
      ...
      if(action==='slot'&&['clover','potion'].includes(game.phase)){
