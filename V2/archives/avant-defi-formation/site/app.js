(() => {
  ...
  function duelRecap(s){
    const d=s.duel;if(!d?.attackRolls.length)return '';
  ...
  function consoleBody(s=game){
    ...
    return `<div class="phase-label">DUEL RÉSOLU</div><p class="outcome">${esc(s.duel.outcome)}</p>${duelRecap(s)}<button class="primary" data-action="next" ${rolling?'disabled':''}>${icon('arrow-right')}Tour suivant</button>`;
  }
