(function (root, factory) {
  ...
  function createEngine(data) {
    ...
    function newGame(deckA,deckB,options={}) {
      ...
      const s={schema:2,phase:'setup',turn:0,round:1,rng:random||1,seed,mode:options.mode||'ai',players:[],log:[],duel:null,lastDuel:null,winner:null,replacing:null};
      [deckA,deckB].forEach((deck,side)=>s.players.push({board:Array(5).fill(null),reserve:deck.map((id,i)=>({uid:side+'-'+i,cardId:id,revived:false,reraise:0,mana:0,physical:0})),dead:[]}));
    ...
    function rollDefense(s,forced) {
      ...
      const lethal=f.attack>f.defense,killed=lethal&&eliminate(s);
    ...
    function assertState(s) {
      if(!s||![1,2].includes(s.schema)||!Array.isArray(s.players)||s.players.length!==2)throw new Error('Sauvegarde incompatible.');
      ...
      if(s.schema===2&&s.phase==='revive')throw new Error('Ancienne phase de resurrection.');
    ...
    return {data,rules,byId,card,clone,dieValue,mean,lineup,validateDeck,newGame,deploy,recall,autoDeploy,start,synergy,elementModifier,lock,rollAttack,rollDefense,next,aiChoice,assertState,restoreGame};
  }
