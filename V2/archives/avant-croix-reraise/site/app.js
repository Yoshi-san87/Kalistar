(() => {
  ...
  const names={retry:'Relance',mana:'Potion',revive:'Résurrection',death:'Mort',dodge:'Esquive',buff_atk:'Buff physique',shield_magic:'Bouclier magique',shield_physical:'Bouclier physique'};
  ...
  const restored=load('game',null);if(restored){try{E.assertState(restored);game=restored;}catch{game=null;}}
  ...
  function render(){
    ...
    $('#app').innerHTML=ui.view==='collection'?collection():arena();icons();persist();scheduleAI();
  }
  ...
  function buffs(p,u){const f=E.synergy(p,u,'faction'),r=E.synergy(p,u,'race');return `${f?`<span>ATK +${f}</span>`:''}${r?`<span>DEF +${r}</span>`:''}${u.mana?`<span class="mana">M +${u.mana}</span>`:''}${u.physical?`<span>PHY +${u.physical}</span>`:''}${u.revived?'<span title="Résurrection déjà consommée">R</span>':''}`;}
  function board(side){
    const p=game.players[side],selectedReserve=p.reserve.find(u=>u.uid===ui.reserve);
    return `<div class="formation">${p.board.map((u,slot)=>{const c=u?E.card(u):null,isA=(game.duel?game.duel.side===side&&game.duel.attackerSlot===slot:game.turn===side&&ui.attacker===slot),isT=(game.duel?game.duel.side!==side&&game.duel.targetSlot===slot:game.turn!==side&&ui.target===slot);return `<div class="slot ${side?'enemy':''} ${isA?'selected':''} ${isT?'target':''} ${!u&&selectedReserve&&E.card(selectedReserve).positions.includes(slot+1)?'compatible':''}"><div class="position-label"><b>P${slot+1}</b><span>${roles[slot]}</span></div><button class="slot-card ${u?'':'empty'}" data-action="slot" data-side="${side}" data-slot="${slot}" aria-label="${side?'Adversaire':'Joueur'} P${slot+1}${c?' '+c.name:', emplacement libre'}">${c?`<img src="${cardImage(c)}" alt="${c.name}" width="538" height="898">`:icon('plus')+'<span>P'+(slot+1)+'</span>'}</button>${c?`<button class="inspect" data-action="detail" data-id="${c.id}" aria-label="Inspecter ${c.name}" title="Inspecter ${c.name}">${icon('scan-eye')}</button>`:''}<div class="slot-buffs">${u?buffs(p,u):''}</div></div>`;}).join('')}</div>`;
  }
  ...
  function dice(which){const d=game.duel||game.lastDuel,side=which==='attack',rolls=d?(side?d.attackRolls:d.defenseRolls):[],die=rolls.at(-1),name=d?(side?d.attackerName:d.targetName):side?'ATK':'DEF';return `<div class="duel-die ${side?'atk':'def'}"><b>${esc(name)}</b><div class="die">${icon('dice-'+(die||6))}</div><small>${die?'D'+die+' · '+format(side?d.attackValue:d.defenseValue):side?'ATK':'DEF'}</small></div>`;}
  function consoleBody(){
    ...
    if(s.phase==='revive')return '<div class="phase-label">RÉSURRECTION</div><h2>Une seconde chance</h2><span class="muted">Carte à ramener en réserve</span>';
  ...
  function reserveZone(){
    let side=game.phase==='replace'?game.replacing:game.phase==='setup'?(ui.setupSide||0):game.turn;
    if(game.mode==='ai'&&side===1&&game.phase!=='revive')side=0;
    const revive=game.phase==='revive',p=game.players[side],units=revive?p.dead.filter(u=>!u.revived):p.reserve;
    return `<div class="reserve-zone"><div class="reserve-heading"><h3>${revive?'Résurrection':'Réserve'} · ${side===0?'Joueur 1':'Joueur 2'} <span class="muted">${units.length}</span></h3>${game.phase==='setup'&&game.mode==='local'?'<button class="text-link" data-action="setup-side">Changer de joueur</button>':''}</div><div class="reserve-cards">${units.map(u=>`<button class="reserve-card ${ui.reserve===u.uid?'selected':''}" data-action="${revive?'revive':'reserve'}" data-uid="${u.uid}" data-side="${side}" title="${E.card(u).name} · ${E.card(u).positions.map(p=>'P'+p).join('/')}" aria-label="${revive?'Ressusciter':'Sélectionner'} ${E.card(u).name}"><img src="${cardImage(E.card(u))}" alt="${E.card(u).name}"></button>`).join('')||'<span class="muted">Aucune carte</span>'}</div></div>`;
  }
  function journal(){
  ...
  function arena(){
    ...
    return `${!storageAvailable?'<div class="storage-note">Sauvegarde navigateur indisponible. Exportez la partie pour la conserver.</div>':''}<div class="game-shell"><div class="arena-toolbar"><div><h1>Arène de Kalistar <span class="round">Échange ${game.round} / 200</span></h1><span class="muted" style="font-size:11px">${esc(game.seed)} · ${game.mode==='ai'?'Adversaire automatique':'Deux joueurs locaux'}</span></div><div class="tools">${ib('save-game','save','Exporter la sauvegarde')}${ib('load-game','upload','Importer une sauvegarde')}${ib('new-game','rotate-ccw','Nouvelle partie')}${ib('rules','scroll-text','Règles')}<input hidden type="file" id="game-file" accept="application/json,.json"></div></div><div class="arena-layout"><section class="battlefield" aria-label="Plateau de jeu">${sideHeading(1)}${board(1)}<div class="duel-console" aria-live="polite">${dice('attack')}<div class="duel-centre">${consoleBody()}</div>${dice('defense')}</div>${sideHeading(0)}${board(0)}${reserveZone()}</section>${journal()}</div></div>`;
  }
  ...
  async function animatedRoll(){
    ...
    rolling=true;clearTimeout(aiTimer);const token=epoch,phase=game.phase;$('#app').classList.add('rolling');
    await new Promise(r=>setTimeout(r,matchMedia('(prefers-reduced-motion:reduce)').matches?30:650));
    $('#app').classList.remove('rolling');rolling=false;
    if(token!==epoch)return;
    act(()=>phase==='attack'?E.rollAttack(game):E.rollDefense(game));
  }
  function scheduleAI(){
    clearTimeout(aiTimer);if(!game||ui.view!=='arena'||game.mode!=='ai'||rolling)return;
    const p=game.phase,token=epoch;
    const active=(p==='choose'&&game.turn===1)||(p==='attack'&&game.turn===1)||(p==='defense'&&game.turn===0)||(p==='revive'&&game.turn===1)||(p==='replace'&&game.replacing===1);
    if(!active)return;
    aiTimer=setTimeout(()=>{if(token!==epoch||ui.view!=='arena')return;
      if(p==='attack'||p==='defense')return animatedRoll();
      act(()=>{if(p==='choose'){const pair=E.aiChoice(game);E.lock(game,...pair);}else if(p==='replace')E.autoDeploy(game,1);else{const eligible=game.players[1].dead.filter(u=>!u.revived).sort((a,b)=>E.mean(E.card(b).atk)-E.mean(E.card(a).atk));E.revive(game,eligible[0].uid);}});
    },p==='choose'?650:450);
  }
  ...
  document.addEventListener('click',event=>{
    ...
    try{
      ...
      if(game?.mode==='ai'&&((action==='revive'&&game.turn===1)||(action==='auto-replace'&&game.replacing===1)))return;
      act(()=>{
        ...
        else if(action==='revive')E.revive(game,b.dataset.uid);
      });
  ...
  document.addEventListener('change',async e=>{
    ...
    if(e.target.id==='deck-file'||e.target.id==='game-file'){
      ...
      try{if(file.size>3000000)throw new Error('Fichier trop volumineux.');const value=JSON.parse(await file.text());
        ...
        else{E.assertState(value);clearTimeout(aiTimer);epoch++;game=value;ui.attacker=ui.target=ui.reserve=null;setView('arena');}
