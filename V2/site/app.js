(async () => {
  ...
  const traitInfo={reraise:{asset:'revive',name:'Reraise'},luck:{asset:'retry',name:'Trèfle'},mana:{asset:'mana',name:'Potion magique'},physical:{asset:'buff_atk',name:'Puissance physique'}};
  const roles=['Tank','DPS physique','Middle','DPS magique','Support'];
  const arenas=[{id:'ruins',name:'Les Ruines de Kalistar',subtitle:'Pierre ancienne · Cristaux',image:'assets/arena.webp'},{id:'sanctuary',name:'Le Sanctuaire submergé',subtitle:'Marbre · Eaux turquoise',image:'assets/arenas/sanctuary.webp'},{id:'forge',name:'La Forge des Cendres',subtitle:'Basalte · Acier · Lave',image:'assets/arenas/forge.webp'}];
  ...
  function preserveLegacy(s){if(s?.schema<4&&!load('game.before-single-trait',null))save('game.before-single-trait',s);}
  const restored=load('game',null);if(restored){try{game=E.restoreGame(restored);preserveLegacy(restored);}catch{game=null;}}
  ...
  function buffs(p,u,side){
    ...
    return `${f?chip('faction','ATK +'+f,'Bonus de faction : ATK +'+f):''}${r?chip('race','DEF +'+r,'Bonus de race : DEF +'+r):''}`;
  }
  function traitBadge(side,u){
    const key=E.trait(u);if(!key)return '';
    const info=traitInfo[key],title=info.name+(u[key]>1?' : +'+u[key]+' ATK':' actif');
  ...
  function board(side){
    ...
    return `<div class="formation cross-formation ${side?'right-cross':'left-cross'}" data-player="${side}">${p.board.map((u,slot)=>{
      ...
      const beneficiary=u&&['clover','potion','heart'].includes(game.phase)&&game.turn===side&&!(game.mode==='ai'&&side===1);
      const currentTrait=E.trait(u),grantName=game.phase==='heart'?'le Reraise':game.phase==='potion'?'la potion magique':'le trèfle';
  ...
  function dice(side){
    const d=['attack','defense','clover','potion','heart','result'].includes(game.phase)?game.duel:null,isAttack=d?d.side===side:game.turn===side,rolls=d?(isAttack?d.attackRolls:d.defenseRolls):[],die=rolls.at(-1);
  ...
  function consoleParticipants(s){
    const d=['attack','defense','clover','potion','heart','result'].includes(s.phase)?s.duel:null,side=d?.side??s.turn;
  ...
  function syncConsole(s=game){
    ...
    node.style.setProperty('--action-color',s.phase==='heart'?'#ff96b7':s.phase==='potion'?'#8adbf5':s.phase==='clover'||s.duel?.autoDefense?'#9bebac':s.phase==='defense'?color(bc)||'#9acdd5':color(ac)||'#d3b7ab');
    node.dataset.phase=s.phase;node.dataset.element=ac?.element||'NONE';
  }
  function recapRow(key,label,value,bonus=true,help=''){
    const symbols={baseAttack:'swords',weapon:'axe',element:'gem',faction:'flag',buff:'sparkles',barrier:'shield-half',baseDefense:'shield',race:'users'};
  ...
  function previewRecap(s){
    ...
    const barrier=pair?(ac.atk.some((v,i)=>typeof v==='number'&&ac.magic.includes(6-i))&&bc.defense.some((v,i)=>typeof v==='number'&&bc.barriers.includes(6-i))?'-'+E.rules.barrier+' possible':0):null;
    const fixed=pair?weapon+element+faction:null,signed=v=>v==null?'…':v>0?'+'+v:String(v);
    return `<section class="duel-recap preview-recap ${!a&&!b?'pending-recap':''}" aria-label="Aperçu des bonus avant les dés"><div class="recap-type">${icon('scan-eye')}<span>Forces engagées</span><small>AVANT JET</small></div>${recapRow('baseAttack','Jet ATK','À lancer',false)}${recapRow('weapon','Arme',weapon,true,pair?ac.weapon+' contre '+bc.weapon:'Selon les deux armes sélectionnées')}${recapRow('element','Cristal',element,true,pair?(ac.element||'Sans cristal')+' contre '+(bc.element||'Sans cristal'):'Selon les deux cristaux sélectionnés')}${recapRow('faction','Faction',faction,true,a?ac.faction+' : cartes alliées présentes sur le plateau':'Faction de l’attaquant')}${recapRow('buff','Jeton',tokens,true,'M : prochaine attaque numérique magique. P : prochaine attaque numérique physique. Aucun jeton consommé avant le jet ATK.')}${recapRow('barrier','Barrière',barrier,true,'Réduction de l’ATK uniquement si la face ATK est magique et la face DEF possède une barrière.')}<div class="recap-defense">${recapRow('baseDefense','Jet DEF','À lancer',false)}${recapRow('race','Race',race,true,b?bc.race+' : cartes alliées présentes sur le plateau':'Race du défenseur')}</div>${scoreTotals(side,'Modif. ATK',signed(fixed),'Bonus DEF',signed(race),true)}<p class="preview-note">Hors dés, jetons et barrière</p></section>`;
  }
  function duelRecap(s){
    const d=['attack','defense','clover','potion','heart','result'].includes(s.phase)?s.duel:null;
    ...
    if(!numeric){
      const support=['retry','mana','revive'].includes(d.attackValue),label=d.attackValue==='death'?'Mort · ignore les scores':s.phase==='heart'?'Reraise à attribuer':s.phase==='clover'?'Trèfle à attribuer':s.phase==='potion'?'Potion magique à attribuer':dieLabel(d.attackValue,true);
      return `<div class="duel-recap special-recap">${support?`<img class="recap-clover" src="${asset('effets',d.attackValue)}" alt="">`:''}<span>${esc(label)}</span>${support?`<span>${d.attackValue==='revive'?'Une vie supplémentaire':d.attackValue==='mana'?'+60 à la prochaine attaque magique':'Seconde chance en défense'}</span><span class="single-trait-note">Un seul trait par carte.<br>Le nouveau remplace l’ancien.</span>`:''}${d.defenseRolls.length?`<span>DEF · ${esc(format(d.defenseValue))}</span>`:''}</div>`;
    }
  ...
  function consoleBody(s=game){
    ...
    }else if(['clover','potion','heart'].includes(s.phase)){
      const potion=s.phase==='potion',heart=s.phase==='heart';label=`${heart?'RERAISE':potion?'POTION MAGIQUE':'TRÈFLE'} · JOUEUR ${s.turn+1}`;title=s.mode==='ai'&&s.turn===1?'Le Veilleur choisit un allié':'Choisir une carte alliée';
      actions=`<span class="clover-choice-status">${icon(heart?'heart-pulse':potion?'flask-conical':'clover')}${heart?'Attribution du Reraise':potion?'Attribution de la potion':'Attribution du trèfle'}</span>`;
    ...
    return `<div class="duel-status"><div class="phase-label">${data.elements[element]?`<img class="console-crystal" src="${asset('cristaux',element)}" alt="Cristal ${element}">`:icon('gem')}<span>${label}</span></div><h2 class="${s.phase==='result'?'outcome':''}">${esc(title)}</h2></div>${duelRecap(s)}<div class="duel-actions">${actions}</div>`;
  }
  ...
  async function animatedTrait(uid){
    if(rolling||!['clover','potion','heart'].includes(game.phase))return;
    const token=epoch,next=E.clone(game);
    try{
      if(game.phase==='heart')E.grantReraise(next,uid);else if(game.phase==='potion')E.grantPotion(next,uid);else E.grantClover(next,uid);
  ...
  function scheduleAI(){
    ...
    const active=second||game.mode==='ai'&&((p==='choose'&&game.turn===1)||(['clover','potion','heart'].includes(p)&&game.turn===1)||(p==='attack'&&game.turn===1)||(p==='defense'&&game.turn===0)||(p==='replace'&&game.replacing===1));
    if(!active)return;
    aiTimer=setTimeout(()=>{if(token!==epoch||ui.view!=='arena')return;
      if(p==='attack'||p==='defense')return animatedRoll();
      if(['clover','potion','heart'].includes(p))return animatedTrait(p==='heart'?E.aiReraiseChoice(game):p==='potion'?E.aiPotionChoice(game):E.aiCloverChoice(game));
  ...
  document.addEventListener('click',event=>{
    ...
    try{
      ...
      if(action==='export-legacy'){const legacy=load('game.before-single-trait',null);if(legacy)return download('Kalistar-avant-trait-unique.json',legacy);return;}
      ...
      if(action==='slot'&&['clover','potion','heart'].includes(game.phase)){
  ...
  document.addEventListener('change',async e=>{
    ...
    if(e.target.id==='deck-file'||e.target.id==='game-file'){
      ...
      try{if(file.size>3000000)throw new Error('Fichier trop volumineux.');const value=JSON.parse(await file.text());
        ...
        else{const imported=E.restoreGame(value);preserveLegacy(value);clearTimeout(aiTimer);epoch++;game=imported;ui.endShown=false;ui.attacker=ui.target=ui.reserve=null;setView('arena');}
  ...
  document.addEventListener('submit',e=>{
    if(e.target.id==='arena-form'){
      ...
      if(game)game.arenaId=id;save('arena',id);$('#arena-dialog').close();render();return;
    }
    if(e.target.id!=='new-game-form')return;
    e.preventDefault();if(rolling)return;
    try{createGame($('#game-mode').value,$('#game-seed').value.trim()||'KALISTAR',new FormData(e.target).get('arena'));$('#new-game-dialog').close();setView('arena');}catch(err){toast(err.message);}
  });
