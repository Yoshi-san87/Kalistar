(() => {
  ...
  const key=c=>c.name.trim().toLocaleUpperCase('fr');
  ...
  function item(group,{deck,favorites,covers,canAdd,db}){
    const c=group.find(c=>c.id===covers[key(c)])||group[0],n=deck.filter(id=>id===c.id).length,stats=db?.career(c.id);
    return `<article class="collection-item character-item" data-character="${esc(c.name)}" style="--character-color:#${data.elements[c.element].color}">
      ...
      <button class="card-open" data-action="detail" data-id="${c.id}" aria-label="Consulter ${esc(c.name+' · '+c.title)}"><img src="assets/cards/${c.slug}.webp" alt="${esc(c.name+' · '+c.title)}" width="538" height="898" loading="lazy"></button>
      ${group.length>1?`<span class="version-count" title="${group.length} versions">${icon('layers-3')}${group.length}</span>`:''}</div>
      <div class="character-caption"><h2>${esc(c.name)}</h2><span>${esc(c.title)}</span></div>
      <div class="version-switch" aria-label="Versions de ${esc(c.name)}">${group.map(v=>`<button data-action="cover-version" data-id="${v.id}" aria-pressed="${v.id===c.id}" title="${esc(v.title)}"><img src="../assets/cristaux/${encodeURIComponent(v.element)}.png" alt="${v.element}"></button>`).join('')}<span>${group.length} version${group.length>1?'s':''}</span></div>
  ...
  function career(id,db,selected='all'){
    ...
    const instances=db.instances(id),s=db.career(id,selected==='all'?null:selected),rate=s.games?Math.round(s.wins/s.games*100):0;
    const cell=(symbol,value,label,cls='')=>`<div class="career-stat ${cls}">${icon(symbol)}<b>${num(value)}</b><span>${label}</span></div>`;
    return `<section class="career-panel"><div class="career-heading"><h3>${icon('medal')}Carrière</h3><label>Exemplaire<select id="career-instance"><option value="all">Tous · ${instances.length} exemplaire${instances.length>1?'s':''}</option>${instances.map(i=>`<option value="${i.id}" ${selected===i.id?'selected':''}>#${i.serial}</option>`).join('')}</select></label></div>
      ...
      <div class="career-history"><h4>Dernières rencontres</h4>${s.history.slice(0,6).map(r=>`<button data-action="history-match" data-id="${esc(r.matchId)}"><b class="${r.winner===r.side?'won':'lost'}">${r.winner==='draw'?'N':r.winner===r.side?'V':'D'}</b><span>${esc(r.seed)}<small>J${r.side+1} · #${r.instanceId.slice(3)} · ${new Date(r.finishedAt).toLocaleDateString('fr-FR')}</small></span><span>${r.kills} K<br>${r.holds} Stop</span>${icon('chevron-right')}</button>`).join('')||'<p class="muted">Aucune rencontre terminée.</p>'}</div></section>`;
  }
